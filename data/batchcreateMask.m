function result = batchcreateMask(inDir, outDir)
%batchcreateMask Batch process images using createMask
% RESULT = batchcreateMask(INDIR, OUTDIR) processes each file in INDIR
% using the function createMask.
%
% The following fields from the output of createMask are written with their
...
if(nargin<1)
    inDir = 'F:\BTP_Thermal\Gopal\Data\Data\Hypo';
end
...
% Process each image using createMask
for imgInd = 1:numel(imds.Files)
    ...
    try
        ...
        % Process
        oneResult = struct();oneResult.output = createMask(im);
